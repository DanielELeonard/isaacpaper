...
func setup_style() -> void:
	...
	normal_style.set_border_width_all(2) # Use proper method instead of direct assignment
	normal_style.border_color = Color.WHITE
	normal_style.corner_radius_top_left = int(BUTTON_SIZE.x / 2)
	normal_style.corner_radius_top_right = int(BUTTON_SIZE.x / 2)
	normal_style.corner_radius_bottom_left = int(BUTTON_SIZE.x / 2)
	normal_style.corner_radius_bottom_right = int(BUTTON_SIZE.x / 2)
